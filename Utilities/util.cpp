...
{
	glColor3f(1.000, 0.271, 0.000); // set the circle color
...
void Torus2d(int x /*Starting position x*/, int y /*Starting position Y*/, float angle /* Starting angle in degrees */, float length /* Length of arc in degrees, >0 */, float radius /* inner radius, >0 */, float width /* width of torus, >0 */, unsigned int samples /* number of circle samples, >=3 */, float *color) 
{
	angle = Deg2Rad(angle);
...
void DrawStringTNR(int x, int y, int width, int height, const string &stringdata,
float*color) 
{
	float fx = (float)x / width * 2 - 1, fy = (float)y / height * 2 - 1;
	DrawStringTNR(fx, fy, stringdata, color);
}

// Function draws a string at given x,y coordinates
void DrawStringTNR(float x, float y, const string& score, float * color) 
{
	//  A pointer to a font style..
	//  Fonts supported by GLUT are: GLUT_BITMAP_8_BY_13,
	//  GLUT_BITMAP_9_BY_15, GLUT_BITMAP_TIMES_ROMAN_10,
...
void DrawString9B15(float x, float y, const string& score, float * color) 
{
	//  A pointer to a font style..
	//  Fonts supported by GLUT are: GLUT_BITMAP_8_BY_13,
	//  GLUT_BITMAP_9_BY_15, GLUT_BITMAP_TIMES_ROMAN_10,
...
void RoundRect(int x, int y, int width, int height, int radius,
	...
	// Separator
	{
		...
		glVertex2i(top_left[0].x, top_left[0].y);glColor3f(1.000, 0.271, 0.000);
...
/* Function to Add Delay in Secs */
void addDelay(int secs)
{
...
void DrawHollowCircle(GLfloat x, GLfloat y, GLfloat r) {
    static const double inc = 3.14/1024;
